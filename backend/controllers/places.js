...
if(req.currentUser?.role !== 'admin') {
    return res.status(403).json({ message: 'You are not allowd to add a place'})
}
...
router.put('/:placeId', async (req, res) => {
    if(req.currentUser?.role !== 'admin') {
        return res.status(403).json({ message: 'You are not allowd to edit a place'})
    }
...
router.delete('/:placeId', async (req, res) => {
    if(req.currentUser?.role !== 'admin') {
        return res.status(403).json({ message: 'You are not allowd to delete a place'})
    }
...
//         currentUser = await userId.findOne({
//             where: {
//                 userId
//             }
//         });
...
//    }
    ...
    if (! req.currentUser) {
        res.status(404).json({ message: `You must be logged in to leave a rant or rave.` })
    }
